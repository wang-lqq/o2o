...
public class WechatLoginController {
	...
	private static final String FRONTEND = "1";//frontend
	private static final String SHOPEND = "2";//shopend
	...
	public String doGet(HttpServletRequest request, HttpServletResponse response) {
		...
		if (null != code) {
			...
			try {
				...
				//通过openId查询WechatAuth,没有则创建
				auth=wechatAuthService.getWechatAuthById(openId);
			} catch (IOException e) {
		...
		//创建
		if(auth==null) {
			//WechatUser创建PersonInfo
			PersonInfo personInfo=WechatUtil.getPersonInfoFromRequest(user);
			//用户点击的是前端展示页面,则注册为普通用户
			if(FRONTEND.equals(roleType)) {
				personInfo.setUserType(1);
			}else if(SHOPEND.equals(roleType)){//若点击店家管理,则注册为店家用户
			...
			//若注册失败
			if(we.getState()!=WechatAuthEnums.SUCCESS.getState()) {
				return null;
			}else {
				auth.getPersonInfo();
