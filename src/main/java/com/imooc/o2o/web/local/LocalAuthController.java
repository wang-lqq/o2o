...
public class LocalAuthController {
	...
	private Map<String, Object> bindLocalAuth(HttpServletRequest request) {
		...
		if (userName != null && password != null && user != null && user.getUserId() != null) {
			...
			try {
				le = localAuthService.bindLocalAuth(localAuth);
			...
			if (le.getState() == LocalAuthEnums.SUCCESS.getState()) {
	...
	@ResponseBody
	private Map<String, Object> verificatio(String username) {
		Map<String, Object> modelMap = new HashMap<String, Object>();
		int effectedNum=localAuthService.verification(username);
		if(effectedNum<=0) {
			//用户名不存在
			modelMap.put("success", true);
		}else {
			//用户名存在
			modelMap.put("errMsg", LocalAuthEnums.HAS_EXISTED.getStateInfo());
			modelMap.put("success", false);
		}
		return modelMap;
	}
